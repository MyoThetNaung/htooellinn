...
export function HexagonBackground({
  ...
  return (
    <div
      ref={containerRef}
      className={cn("relative min-h-screen overflow-hidden bg-neutral-950", className)}
    >
      {/* Hexagon grid */}
            ...
            >
              {Array.from({ length: grid.cols }).map((_, colIndex) => (
                <div
                  ...
                  style={hexagonStyle as CSSProperties}
                />
              ))}
            </div>
          );
      ...
      <div
        ...
        style={{
          background: `radial-gradient(ellipse at 30% 20%, ${glowColor.replace(
            "0.6",
            "0.15"
          )} 0%, transparent 50%),
                       radial-gradient(ellipse at 70% 80%, ${glowColor.replace(
                         "0.6",
                         "0.1"
                       )} 0%, transparent 50%)`,
        }}
